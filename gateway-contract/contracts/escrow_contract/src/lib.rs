...
impl EscrowContract {
    ...
    /// Returns the scheduled payment identified by its payment ID.
    ///
    /// ### Arguments
    /// - `payment_id`: The unique identifier of the scheduled payment.
    ///
    /// ### Returns
    /// - `Some(ScheduledPayment)` if the payment exists.
    /// - `None` if the payment does not exist.
