...
const VK_ALPHA_G1: ([u8; 32], [u8; 32]) = (
    ...
    [
        0x09, 0xf8, 0xe7, 0xd6, 0xc5, 0xb4, 0xa3, 0x92, 0x81, 0x70, 0x6f, 0x5e, 0x4d, 0x3c, 0x2b,
        0x1a, 0x09, 0xf8, 0xe7, 0xd6, 0xc5, 0xb4, 0xa3, 0x92, 0x81, 0x70, 0x6f, 0x5e, 0x4d, 0x3c,
        0x2b, 0x1a,
    ],
);
...
const VK_BETA_G2: (([u8; 32], [u8; 32]), ([u8; 32], [u8; 32])) = (
    (
        ...
        [
            0x19, 0x08, 0xf7, 0xe6, 0xd5, 0xc4, 0xb3, 0xa2, 0x91, 0x80, 0x7f, 0x6e, 0x5d, 0x4c,
            0x3b, 0x2a, 0x19, 0x08, 0xf7, 0xe6, 0xd5, 0xc4, 0xb3, 0xa2, 0x91, 0x80, 0x7f, 0x6e,
            0x5d, 0x4c, 0x3b, 0x2a,
        ],
    ),
    (
        [
            0x3b, 0x4c, 0x5d, 0x6e, 0x7f, 0x80, 0x91, 0xa2, 0xb3, 0xc4, 0xd5, 0xe6, 0xf7, 0x08,
            0x19, 0x2a, 0x3b, 0x4c, 0x5d, 0x6e, 0x7f, 0x80, 0x91, 0xa2, 0xb3, 0xc4, 0xd5, 0xe6,
            0xf7, 0x08, 0x19, 0x2a,
        ],
        [
            0x2a, 0x19, 0x08, 0xf7, 0xe6, 0xd5, 0xc4, 0xb3, 0xa2, 0x91, 0x80, 0x7f, 0x6e, 0x5d,
            0x4c, 0x3b, 0x2a, 0x19, 0x08, 0xf7, 0xe6, 0xd5, 0xc4, 0xb3, 0xa2, 0x91, 0x80, 0x7f,
            0x6e, 0x5d, 0x4c, 0x3b,
        ],
    ),
);
...
pub fn groth16_verify(
    ...
    if !validate_g1(&proof_a.0, &proof_a.1) {
        return false;
    }
    if !validate_g1(&proof_c.0, &proof_c.1) {
        return false;
    }
    if !validate_g1(&proof_b.0 .0, &proof_b.0 .1) {
        return false;
    }

    // 2. Validate public inputs are in-field
    for inp in public_inputs {
        if !field_element_valid(inp) {
            return false;
        }
    }
...
pub fn verify_proof_bytes(_env: &Env, proof_bytes: &[u8], public_inputs: &[[u8; 32]]) -> bool {
    if proof_bytes.len() != 256 {
        return false;
    }

    let mut a_x = [0u8; 32];
    a_x.copy_from_slice(&proof_bytes[0..32]);
    let mut a_y = [0u8; 32];
    a_y.copy_from_slice(&proof_bytes[32..64]);
    let mut b_x0 = [0u8; 32];
    b_x0.copy_from_slice(&proof_bytes[64..96]);
    let mut b_x1 = [0u8; 32];
    b_x1.copy_from_slice(&proof_bytes[96..128]);
    let mut b_y0 = [0u8; 32];
    b_y0.copy_from_slice(&proof_bytes[128..160]);
    let mut b_y1 = [0u8; 32];
    b_y1.copy_from_slice(&proof_bytes[160..192]);
    let mut c_x = [0u8; 32];
    c_x.copy_from_slice(&proof_bytes[192..224]);
    let mut c_y = [0u8; 32];
    c_y.copy_from_slice(&proof_bytes[224..256]);
